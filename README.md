...
- **图寻镜像模式（`--mirror`）**：本地反向代理把图寻整站镜像到 `http://127.0.0.1:8001`——**免装证书、免接管系统代理**，浏览器直接访问本地端口做题；游戏页面内注入悬浮窗（原点/目前/答案 + 开关设置全在网页上调）；CDN 资产与 API 全部经镜像改写，Cookie 会话自动托管
...
# 浏览器访问 http://127.0.0.1:8001 做题，页面内悬浮窗显示原点/目前/答案并可调设置
...
| `mirror_enabled` | 启动时自动开启图寻镜像（也可用 `--mirror` 临时开启） | `false` |
| `mirror_port` | 图寻镜像端口（浏览器访问 `http://127.0.0.1:该端口` 做题） | `8001` |
| `control_port` | 镜像悬浮窗的状态/设置 API 端口（仅本机监听） | `18080` |
...
| 产物 | 说明 |
|---|---|
| `TuxunHelper-Realtime.exe`（约 38 MB） | 实时取点全套：GUI 地图 + 本地代理 + 诱饵免疫 + Cookie 自动录入 + 答案揭示 |
...
├── tuxun_proxy.py    # 实时取点模式入口（本地代理 / 诱饵判定 / Cookie 自动录入）
