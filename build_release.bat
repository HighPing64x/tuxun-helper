...
REM    TuxunHelper-Lite.exe      Lite 外置小窗（--lite，pywebview 顶置显示坐标，无控制台）
REM    TuxunHelper-AI.exe        AI 分析 / 复盘 / 抽奖（命令行）
REM  2.0 起为全网页版：web\ 目录（index.html/lite.html 前端 + vendor Leaflet
REM  + tutorial.md 教程）打包进 exe；Lite 目标需 pywebview（--windowed 隐掉控制台）
...
if not exist .build-venv\Scripts\python.exe (
    ...
    .build-venv\Scripts\pip install -q requests python-dotenv pillow "mitmproxy>=10" "bcrypt==4.0.1" "rich>=13" "pywebview>=5" pyinstaller websocket-client -i https://pypi.tuna.tsinghua.edu.cn/simple
)
...
echo [构建] Lite 外置小窗 TuxunHelper-Lite.exe ...
.build-venv\Scripts\pyinstaller --noconfirm --onefile --windowed --clean ^
  --name TuxunHelper-Lite ^
  --add-data "web;web" ^
  --collect-all mitmproxy ^
  --collect-submodules mitmproxy ^
  --collect-all mitmproxy_rs ^
